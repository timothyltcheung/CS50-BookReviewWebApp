...
def books():
    ...
    books = db.execute("SELECT * FROM books WHERE title LIKE :input OR author LIKE :input OR isbn LIKE :input", {"input": '%' + identifier + '%'}).fetchall()
    if books:
...
def book(isbn):
    if request.method == "POST":
        ...
        try:
            db.execute("INSERT INTO reviews (user_id, isbn, review_score, review) VALUES (:user_id, :isbn, :review_score, :review)", {"user_id": session["user_id"][0], "isbn": isbn, "review_score": review_score, "review": review})
            db.commit()
        except:
